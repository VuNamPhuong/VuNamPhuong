...
3/ Draw a static pool_ball sprite to the screen
...
#define WINDOW_ICON_PATH L"pool_ball.ico"
...
#define TEXTURE_PATH_pool_ball L"pool_ball.png"
#define pool_ball_START_X 8.0f
#define pool_ball_START_Y 200.0f

#define pool_ball_START_VX 0.2f
#define pool_ball_START_VY 0.2f

#define pool_ball_WIDTH 16.0f
#define pool_ball_HEIGHT 16.0f


ID3D10Texture2D* texpool_ball = NULL;				// Texture object to store pool_ball image
ID3DX10Sprite* spriteObject = NULL;				// Sprite handling object 

D3DX10_SPRITE spritepool_ball;

float pool_ball_x = pool_ball_START_X;
float pool_ball_vx = pool_ball_START_VX;
float pool_ball_y = pool_ball_START_Y;
float pool_ball_vy = pool_ball_START_VY;
...
/*
	Load game resources. In this example, we only load a pool_ball image
*/
void LoadResources()
{
	...
	pD3D10Resource->QueryInterface(__uuidof(ID3D10Texture2D), (LPVOID*)&texpool_ball);
	...
	pD3DDevice->CreateShaderResourceView(texpool_ball, &SRVDesc, &gSpriteTextureRV);

	// Set the sprite’s shader resource view
	spritepool_ball.pTexture = gSpriteTextureRV;

	// top-left location in U,V coords
	spritepool_ball.TexCoord.x = 0;
	spritepool_ball.TexCoord.y = 0;

	// Determine the texture size in U,V coords
	spritepool_ball.TexSize.x = 1.0f;
	spritepool_ball.TexSize.y = 1.0f;

	// Set the texture index. Single textures will use 0
	spritepool_ball.TextureIndex = 0;

	// The color to apply to this sprite, full color applies white.
	spritepool_ball.ColorModulate = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
...
{
	//Uncomment the whole function to see the pool_ball moves and bounces back when hitting left and right edges
	pool_ball_x++;
	pool_ball_x += pool_ball_vx*dt; 

	pool_ball_y++;
	pool_ball_y += pool_ball_vy * dt;

	// NOTE: BackBufferWidth is indeed related to rendering!!
	float right_edge = BackBufferWidth - pool_ball_WIDTH;
	float bottom_edge = BackBufferHeight - pool_ball_HEIGHT;

	if (pool_ball_x < 0 || pool_ball_x > right_edge) {

		pool_ball_vx = -pool_ball_vx;

		//	//Why not having these logics would make the pool_ball disappear sometimes?  
			if (pool_ball_x < 0)
			{
				pool_ball_x = 0;
			}
			else if (pool_ball_x > right_edge )
			{
				pool_ball_x = right_edge;
			}
	}

	if (pool_ball_y < 0 || pool_ball_y > bottom_edge) {

		pool_ball_vy = -pool_ball_vy;
		...
		else if (pool_ball_y > bottom_edge)
		{
			pool_ball_y = bottom_edge;
		}
	}
}
...
void Render()
{
	if (pD3DDevice != NULL)
	{
		...
		D3DXMatrixTranslation(&matTranslation, pool_ball_x, (BackBufferHeight - pool_ball_y), 0.1f);

		// Scale the sprite to its correct width and height
		D3DXMATRIX matScaling;
		D3DXMatrixScaling(&matScaling, pool_ball_WIDTH, pool_ball_HEIGHT, 1.0f);

		// Setting the sprite’s position and size
		spritepool_ball.matWorld = (matScaling * matTranslation);

		spriteObject->DrawSpritesImmediate(&spritepool_ball, 1, 0, 0);

		// Finish up and send the sprites to the hardware
		spriteObject->End();

		//DebugOutTitle((wchar_t*)L"%s (%0.1f,%0.1f) v:%0.1f", WINDOW_TITLE, pool_ball_x, pool_ball_y, pool_ball_vx);
